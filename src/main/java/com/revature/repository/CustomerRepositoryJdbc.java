...
public class CustomerRepositoryJdbc implements CustomerRepository
{
	...
	try (Connection connection = ConnectionUtil.getConnection())
	{
	    ...
	    while (result.next())
	    {
		return new Customer(result.getInt("C_ID"), result.getString("C_FIRST_NAME"),
			result.getString("C_LAST_NAME"), result.getString("C_USERNAME"),
	...
	try (Connection connection = ConnectionUtil.getConnection())
	{
	    ...
	    while (result.next())
	    {
		customerList.add(new Customer(result.getInt("C_ID"), result.getString("C_FIRST_NAME"),
			result.getString("C_LAST_NAME"), result.getString("C_USERNAME"),
			result.getString("C_PASSWORD")));
	    }
